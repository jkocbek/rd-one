...
export async function __e2eShouldSuccessfullyResetPassword(app: INestApplication): Promise<void> {
  const email = 'auth-password.reset.success@example.com';
  const code = testGenerateRandomForgotPasswordCode();

  const user = await UserFixture.createUser(app, email);
  await PasswordForgotFixture.createCode(app, user.id, code);
