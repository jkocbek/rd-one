...
export async function __e2eShouldSuccessfullyRefreshExpiredUserSession(app: INestApplication): Promise<void> {
  const email = 'auth.refresh.success.refreshExpiredUserSession@example.com';
  ...
  const jti = await UserFixture.getAccessTokenJwt(loginData.accessToken);
  await UserSessionFixture.expireUserSessionByJti(app, jti);
