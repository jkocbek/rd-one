...
export async function __e2eShouldSuccessfullyRefreshExpiredAccessToken(app: INestApplication): Promise<void> {
  const email = 'auth.refresh.success.refreshExpiredAccessToken@example.com';
  ...
  const expiredAccessToken = await UserFixture.expireAccessToken(loginData.accessToken);
