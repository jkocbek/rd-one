...
import { loadConfig, readConfig } from './config.helpers';
 ...
 * @param cls
 */
export function getRawConfig<T extends object>(cls: ClassConstructor<T>): T {
  ...
  return loadConfig(cls, rawConfig[configKey]);
}
...
export function getConfig<T extends object>(cls: ClassConstructor<T>): T {
  ...
  const config = loadConfig(cls, rawConfig[configKey]);
